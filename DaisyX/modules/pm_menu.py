...
async def get_start_func(message, strings, edit=False):
    ...
    buttons.add(
        ...
        InlineKeyboardButton(
            strings["btn_source"], url="https://github.com/LelouchRobot"
        ),
    )
    buttons.add(
        InlineKeyboardButton(strings["btn_channel"], url="https://t.me/BlackPrinceUpdates"),
        InlineKeyboardButton(
            strings["btn_group"], url="https://t.me/BlackPrinceSupport"
        ),
    )
    buttons.add(
        InlineKeyboardButton(
            "➕ Add Lelouch to your group",
            url=f"https://telegram.me/LelouchRobot?startgroup=true",
        )
    )
...
async def help_cmd_g(message, strings):
    ...
    button = InlineKeyboardMarkup().add(
        InlineKeyboardButton(text=text, url="https://t.me/LelouchRobot?start")
    )
    await message.reply(strings["help_header"], reply_markup=button)
...
async def helpmenu_callback(query, callback_data=None, **kwargs):
    ...
    msg += f"{MOD_HELP[mod]}"
    button = InlineKeyboardMarkup().add(
        InlineKeyboardButton(text="🔙 Go Back", callback_data="get_help")
    )
    with suppress(MessageNotModified):
